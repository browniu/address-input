...
export default class ExampleComponent extends Component {
  ...
  render() {
    ...
    const {rootClass,placeHolder=['省-市-区','详细地址（精确到门牌号）']} = this.props
    return (
      <div className={cx(`${prefix}root`,rootClass)}>
        <div className={`${prefix}main`}>
          <div className={`${prefix}inputGroup`} onClick={() => this.clickAreaInput()}>
            <p className={`${prefix}input`}>{areaInputValue || <span>{placeHolder[0]||'省-市-区'}</span>}</p>
          </div>
          <div className={`${prefix}inputGroup`} onClick={() => this.clickDetailInput()}>
            <p className={`${prefix}input`}>{detailInputValue || <span>{placeHolder[1]||'详细地址（精确到门牌号）'}</span>}</p>
          </div>
        </div>
